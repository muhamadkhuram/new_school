...
class MovieDetailsController extends GetxController {
  ...
  String? extractIframeUrl(String htmlContent) {
    RegExp regExp = RegExp(r'src="([^"]+)"');
    Match? match = regExp.firstMatch(htmlContent);
    return match?.group(1);
  }

  String convertToEmbedUrl(String youtubeUrl) {
    Uri? uri = Uri.tryParse(youtubeUrl);
    ...
    if (videoId == null || videoId.isEmpty) {
      throw ArgumentError("Invalid YouTube video URL");
    }

    return "https://www.youtube.com/embed/$videoId";
  }

  Future<void> initializePlayer(String s,
      {required Duration playDuration}) async {
    //  await loadSubtitles();
    log(s);
    String? url =
        s.contains("youtube.com") ? convertToEmbedUrl(s) : extractIframeUrl(s);
    // String? url = s;
    ...
    webcontroller = WebViewController()
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (String url) {
            // Inject fullscreen detection JavaScript
            webcontroller?.runJavaScript('''
              document.addEventListener('fullscreenchange', function() {
                if (document.fullscreenElement) {
                  VideoFullscreen.postMessage('enter');
                } else {
                  VideoFullscreen.postMessage('exit');
                }
              });
          ...
          onNavigationRequest: (NavigationRequest request) {
            if (request.url.startsWith('https://iframe.mediadelivery.net/')) {
      ...
      ..loadRequest(Uri.parse(url!), headers: {
        'Referer': '${UrlContainer.baseUrl}',
      });

    // videoPlayerController = VideoPlayerController.networkUrl(
    //     Uri.parse(
    //         "https://iframe.mediadelivery.net/embed/362602/26ffde53-4c81-4111-bbd9-c8f3d661a97f?autoplay=true&loop=false&muted=false&preload=false&responsive=true"),
    ...
    //                                     getSubtitlesData(subtitleString));
    //                                 Get.back();
    //                               },
    //                               child: Container(
    ...
    //                                   children: [
    //                                     Text(
    //                                         subTitleLangList[index].language ??
    //                                             '',
    //                                         style: mulishSemiBold.copyWith(
    ...
    //       playedColor: const Color.fromRGBO(255, 255, 255,
    //           1), // Color for the played portion of the progress bar
    //       handleColor: const Color.fromRGBO(255, 255, 255,
    //           1), // Colo// r for the handle (thumb) of the progress bar
    //       backgroundColor: const Color.fromRGBO(156, 156, 156,
    //           0.5), // Color for the background of the progress bar
    //       bufferedColor: const Color.fromRGBO(255, 255, 255,
    //           0.3), // Color for the buffered portion of the progress bar
    //     ),
    //   );
  ...
  changeVideoDuration(Duration duration) {
  ...
  Future<dynamic> loadVideoUrl() async {
    ...
    if (model.statusCode == 200) {
      ...
      if (responseModel.data != null &&
        ...
        //set this to true here to make the rent feature work
        isNeedToRent = true;
        ...
        if (isAuthorized()) {
          showSubscribeDialog(Get.context!,itemId, movieDetails.data?.item?.rentPrice!.toString()??"0");
        } else {
  ...
  Future<void> loadVideoDetails() async {
    ...
    final videoDetailsResponseModel = await movieDetailsRepo
        .getVideoDetails(itemId, episodeId: episodeId == -1 ? -1 : episodeId);

    if (videoDetailsResponseModel.statusCode == 200) {
      final videoDetailsModel = VideoDetailsResponseModel.fromJson(
          jsonDecode(videoDetailsResponseModel.responseJson));
      if (videoDetailsModel.status?.toLowerCase() == 'success' &&
          videoDetailsModel.data?.item != null) {
        ...
        if (videoDetailsModel.data?.episodes != null &&
            videoDetailsModel.data!.episodes!.isNotEmpty) {
        ...
        if (videoDetailsModel.remark == 'unauthorized') {
          ...
          if (isAuthorized()) {
            showSubscribeDialog(Get.context!, itemId, fromDetails: true,movieDetails.data?.item?.rentPrice!.toString()??"0");
          } else {
            showLoginDialog(Get.context!, fromDetails: true);
          }
          CustomSnackbar.showCustomSnackbar(
              errorList: [MyStrings.purchaseAPlan], msg: [], isError: true);
          ...
          CustomSnackbar.showCustomSnackbar(
            errorList: [
              CustomValueConverter.removeQuotationAndSpecialCharacterFromString(
                  videoDetailsModel.message?.error?.toString() ??
      ...
      playVideoLoading = false;
      CustomSnackbar.showCustomSnackbar(
          errorList: [videoDetailsResponseModel.message],
          msg: [],
          isError: true);
    }
  ...
  void addInWishList() async {
    ...
    if (model.statusCode == 200) {
      ...
      if (m.status == 'success') {
        ...
        CustomSnackbar.showCustomSnackbar(
            errorList: [], msg: [m.message?.success ?? ''], isError: false);
      } else {
        CustomSnackbar.showCustomSnackbar(errorList: [
          m.message?.error ??
              (isFavourite
                  ? MyStrings.failToRemoveInWishlist
                  : MyStrings.failToAddInWishlist)
        ], msg: [], isError: true);
      }
    } else {
      CustomSnackbar.showCustomSnackbar(
...
//1 march
  ...
  void rentVideo() async {
    ...
    update();
    try {
      ResponseModel response = await movieDetailsRepo
          .buyPlan(movieDetails.data?.item?.id.toString() ?? '-1');
  ...
  Future<void> createParty(
    ...
    ResponseModel responseModel =
        await movieDetailsRepo.createWatchParty(episodeId, itemId: itemId);
    if (responseModel.statusCode == 200) {
      CreatePartyResponseModel model = CreatePartyResponseModel.fromJson(
  ...
  bool isAuthorized() {
    String token = movieDetailsRepo.apiClient.sharedPreferences
