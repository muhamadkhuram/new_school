...
class MovieDetailsRepo {
  ...
  Future<dynamic> getVideoDetails(int itemId, {int episodeId = -1}) async {
    ...
    if (episodeId == -1 || episodeId == 0) {
      url =
          '${UrlContainer.baseUrl}${token.isEmpty ? UrlContainer.watchVideoEndPoint : UrlContainer.watchVideoPaidEndPoint}=$itemId';
    } else {
      url =
          '${UrlContainer.baseUrl}${token.isEmpty ? UrlContainer.watchVideoEndPoint : UrlContainer.watchVideoPaidEndPoint}=$itemId&episode_id=$episodeId';
    }
    if (token.isEmpty) {
  ...
  Future<dynamic> getVideoData(int itemId, {int episodeId = -1}) async {
    ...
    if (token.isEmpty) {
      if (episodeId == -1) {
        url =
            '${UrlContainer.baseUrl}${UrlContainer.playVideoEndPoint}=$itemId';
      } else {
        url =
            '${UrlContainer.baseUrl}${UrlContainer.playVideoEndPoint}=$itemId&episode_id=$episodeId';
      }
      response = await apiClient.request(url, Method.getMethod, null);
    } else {
      if (episodeId == -1) {
        url =
            '${UrlContainer.baseUrl}${UrlContainer.playVideoPaidEndPoint}=$itemId';
      } else {
        url =
            '${UrlContainer.baseUrl}${UrlContainer.playVideoPaidEndPoint}=$itemId&episode_id=$episodeId';
      }
      response = await apiClient.request(url, Method.getMethod, null,
          passHeader: true);
    }
  ...
  Future<dynamic> checkWishlist(int itemId, {int episodeId = -1}) async {
    String url =
        '${UrlContainer.baseUrl}${UrlContainer.checkWishlistEndpoint}${episodeId < 1 ? 'item_id=${itemId.toString()}' : 'episode_id=${episodeId.toString()}'}';
  ...
  Future<dynamic> addInWishList(int itemId, {int episodeId = -1}) async {
    ...
    if (episodeId == -1) {
      url =
          '${UrlContainer.baseUrl}${UrlContainer.addInWishlistEndPoint}?item_id=$itemId';
    } else {
      url =
          '${UrlContainer.baseUrl}${UrlContainer.addInWishlistEndPoint}?episode_id=$episodeId';
    }
    response =
  ...
  Future<dynamic> removeFromWishList(int itemId, {int episodeId = -1}) async {
    ...
    if (episodeId == -1) {
      url =
          '${UrlContainer.baseUrl}${UrlContainer.removeFromWishlistEndPoint}item_id=$itemId';
    } else {
      url =
          '${UrlContainer.baseUrl}${UrlContainer.removeFromWishlistEndPoint}episode_id=$episodeId';
    }
    response =
        await apiClient.request(url, Method.postMethod, null, passHeader: true);
    return response;
  ...
  Future<ResponseModel> createWatchParty(
    ...
    if (episodeId == "-1") {
      url =
          "${UrlContainer.baseUrl}${UrlContainer.createPartyEndPoint}?item_id=$itemId&";
    } else {
      url =
          "${UrlContainer.baseUrl}${UrlContainer.createPartyEndPoint}?item_id=$itemId&episode_id=$episodeId";
    }
